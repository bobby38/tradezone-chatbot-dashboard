...
export async function POST(request: NextRequest) {
  try {
    ...
    const voiceModel = settings.voiceModel || 'gpt-realtime-mini'
    const voice = settings.voice || 'alloy'

    console.log(`[Realtime API] Initializing voice session: ${sessionId}, Model: ${voiceModel}, Voice: ${voice}`)
    ...
    return NextResponse.json({
      ...
      config: {
        ...
        instructions: settings.systemPrompt || 'You are Izacc, a friendly assistant from TradeZone.sg',
        turnDetection: {
          type: 'server_vad',
          threshold: 0.5,

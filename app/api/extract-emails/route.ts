...
export async function POST(req: NextRequest) {
  try {
    ...
    if (extractNew) {
      ...
      if (chatError) {
        ...
        for (const chat of chatLogs) {
          ...
          promptEmails.forEach((email) => {
            const classification = classifyEmailContext(
              chat.prompt || "",
              email,
            );
      ...
      if (submissionError) {
        ...
        for (const submission of submissions) {
          ...
          const contentEmails = extractEmailsFromText(
    ...
    if (getAllError && getAllError.code !== "PGRST116") {
...
function classifyEmailContext(
  ...
  if (
    lowerContext.includes("trade") ||
    lowerContext.includes("sell") ||
  ...
  if (
    ...
    lowerContext.includes("reach") ||
    lowerContext.includes("get in touch")
  ) {
  ...
  if (
    ...
    lowerContext.includes("device") ||
    lowerContext.includes("iphone") ||
    lowerContext.includes("samsung")
  ) {
...
function classifySubmissionContext(metadata: any): {
  type: string;
  confidence: number;
} {
  if (
    ...
    metadata.body_condition
  ) {
    return { type: "trade_in_form", confidence: 1.0 };
  }

  if (
    metadata.subject?.toLowerCase().includes("trade") ||
    metadata.message?.toLowerCase().includes("trade")
  ) {
    return { type: "trade_inquiry", confidence: 0.9 };
  }
...
function generateEmailStats(emails: ExtractedEmail[]) {
  ...
  emails.forEach((email) => {
    // Count by source
    stats.bySource[email.source_type] =
      (stats.bySource[email.source_type] || 0) + 1;

    // Count by classification
    stats.byClassification[email.classification] =
      (stats.byClassification[email.classification] || 0) + 1;

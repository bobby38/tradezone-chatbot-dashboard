...
export async function GET(req: NextRequest) {
  try {
    ...
    try {
      ...
      console.error('GA top-pages runReport error:', err?.response?.data || err?.message || err)
      if (debug) {
      ...
      return NextResponse.json({ error: 'Failed to fetch GA top pages' }, { status: 500 })
    }
    ...
    return NextResponse.json({ data: rows, timestamp: new Date().toISOString() })
  } catch (error: any) {
    console.error('GA top-pages error:', error?.response?.data || error?.message || error)

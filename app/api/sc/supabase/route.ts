...
export async function GET(req: NextRequest) {
  try {
    ...
    if (summaryError) {
      console.error(
        "Error fetching SC summary data from Supabase:",
        summaryError,
      );
    ...
    try {
      let queryBuilder = supabase
        ...
        .select(
          "query, page, clicks, impressions, ctr, position, device, country",
        )
        .in("site", candidates)
        .gte("date", startDate)
        .lte("date", endDate)
      ...
      if (searchQ) {
        ...
        queryBuilder = queryBuilder.or(
          `query.ilike.%${searchQ}%,page.ilike.%${searchQ}%`,
        );
      }
    ...
    try {
      ...
      if (searchQ) {
        pageBuilder = pageBuilder.or(
          `page.ilike.%${searchQ}%,query.ilike.%${searchQ}%`,
        );
      }
    ...
    try {
      ...
      if (searchQ) {
        perfBuilder = perfBuilder.or(
          `query.ilike.%${searchQ}%,page.ilike.%${searchQ}%`,
        );
      }
      const { data: perfData } = await perfBuilder.limit(
        Math.max(pageSizeParam * 5, 500),
      );

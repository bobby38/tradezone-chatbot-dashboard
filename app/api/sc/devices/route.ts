...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    console.error('SC devices error:', err?.response?.data || err?.message || err)
    if (debug) return NextResponse.json({ error: 'SC devices failed', details: err?.response?.data || { message: err?.message || String(err) } }, { status: err?.response?.status || 500 })
    return NextResponse.json({ error: 'Failed to fetch Search Console devices' }, { status: 500 })
  }
}

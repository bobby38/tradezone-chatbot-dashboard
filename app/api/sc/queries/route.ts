...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (q) filters.push({ dimension: 'query', operator: 'contains', expression: q })
    ...
    console.error('SC queries error:', err?.response?.data || err?.message || err)
    if (debug) return NextResponse.json({ error: 'SC queries failed', details: err?.response?.data || { message: err?.message || String(err) } }, { status: err?.response?.status || 500 })
    return NextResponse.json({ error: 'Failed to fetch Search Console queries' }, { status: 500 })
  }
}

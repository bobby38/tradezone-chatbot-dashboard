...
test.describe("Trade-In Price-First Flow - Critical Tests", () => {
  test("Xbox Series S upgrade query should show price BEFORE asking condition", async ({
    request,
  }) => {
    ...
    const mentionsUpgrade =
      ...
      responseText.includes("top-up") ||
      responseText.includes("topup");

    expect(mentionsUpgrade).toBeTruthy();
  });

  test("PS5 trade-in query should quote price immediately", async ({
    request,
  }) => {
    ...
    // Should mention PS5
    const mentionsPS5 =
      responseText.includes("ps5") || responseText.includes("playstation 5");

    expect(mentionsPS5).toBeTruthy();
  });

  test("Condition question should only come AFTER user confirms interest", async ({
    request,
  }) => {
    ...
    const firstResponse = await request.post(
      `${API_BASE_URL}/api/chatkit/agent`,
      {
        headers: {
          "Content-Type": "application/json",
          "X-API-Key": API_KEY,
  ...
  test("Trade-in price accuracy - Xbox Series S should be S$150", async ({
    request,
  }) => {
    const response = await request.post(`${API_BASE_URL}/api/chatkit/agent`, {
      headers: {
    ...
    const hasCorrectPrice =
      responseText.includes("150") || responseText.includes("s$150");

    expect(hasCorrectPrice).toBeTruthy();
  });

  test("Trade-in price accuracy - Xbox Series X should be S$350", async ({
    request,
  }) => {
    const response = await request.post(`${API_BASE_URL}/api/chatkit/agent`, {
      headers: {
    ...
    const hasCorrectPrice =
      responseText.includes("350") || responseText.includes("s$350");

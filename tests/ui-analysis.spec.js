...
test.describe('Dashboard UI Analysis', () => {
  ...
  test('Analyze dashboard layout and navigation', async ({ page }) => {
    ...
    const notificationCenter = await page.locator('[data-testid="notification-center"], .notification-center, button:has(svg[data-testid="bell"])').first();
  ...
  test('Test mobile responsiveness', async ({ page }) => {
    ...
    const mobileMenuButton = await page.locator('button:has(svg), [aria-label*="menu"], [data-testid="mobile-menu"]').first();
  ...
  test('Check accessibility and interaction', async ({ page }) => {
    ...
    const bellIcon = await page.locator('svg').filter({ hasText: /bell|notification/i }).first();

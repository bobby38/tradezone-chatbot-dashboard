...
test.describe('Agent Tools API', () => {
  ...
  test('should successfully use vectorSearch for an existing product', async ({ request }) => {
  ...
  test('should fall back to perplexitySearch for a non-existing product', async ({ request }) => {
    const response = await request.post(API_ENDPOINT, {
      data: {
    ...
    expect(response.ok()).toBeTruthy();
    const responseBody = await response.json();
    expect(responseBody.response).not.toContain('Web search is currently unavailable');
    expect(responseBody.response).not.toContain('I encountered an error searching the website');
  });
});
